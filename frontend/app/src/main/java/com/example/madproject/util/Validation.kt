...
object Validation {
    ...
    fun validateGroup(group : NewGroup) : Map<String, ValidationResult>{
        ...
        if (group.name.isNullOrEmpty()) {
            ...
            out += "name" to ValidationResult(false, "Name must be between 8 and 20 characters long")
        } else if (!Regex("^[a-zA-Z0-9_-]*\$").matches(group.name!!)) {
            out += "name" to ValidationResult(false, "Name can only contain letters, numbers, underscores and dots")
        } else {
        ...
        if (!group.description.isNullOrEmpty() && group.description!!.length > 150){
            out += "description" to ValidationResult(false, "Description cannot be longer than 150 characters")
        }
        else{
